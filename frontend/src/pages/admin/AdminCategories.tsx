...
export default function AdminCategories() {
  ...
  const remove = async (item: string) => { if (!(await confirm(`Xóa danh mục ${item}?`))) return; const next = categories.filter((x) => x !== item); setCategories(next); localStorage.setItem(KEY, JSON.stringify(next)); };
